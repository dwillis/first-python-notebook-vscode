...
```python
# %%
print("Accident list information:")
...
```python
# %%
print("Accident list info after date conversion:")
print(accident_list.info())
```
...
```python
# %%
# Update the groupby to include make information
accident_counts = accident_list.groupby(["latimes_make", "latimes_make_and_model"]).size().rename("accidents").reset_index()
print("Updated accident counts with make information")
```

Rerun __all__ of the code sections after that one to update everything you're working with and add the new column.

```{note}
Remember: If we change a variable, future code sections that use that variable won't change unless we run them again. When you go back and make these changes, make sure to run all of the code sections that come after them as well, otherwise you may not get the results you're expecting.

This is one reason that it can be good to clear outputs and rerun your analysis every so often. If you've been going back and forth editing code sections and tweaking your analysis, you may have saved variables in memory that are no longer accurate. One way to do that is to restart VS Code's Python environment and rerun the whole analysis to make sure everything still runs as you expect it to.
```
...
```python
# %%
# Chart with color coding by manufacturer
alt.Chart(merged_list).mark_bar().encode(
    x="per_100k_hours",
    y=alt.Y("latimes_make_and_model").sort("-x"),
    color="latimes_make"
...
```python
# %%
# Create a boolean test for Robinson helicopters
print("Robinson test results:")
print(merged_list["latimes_make"] == "ROBINSON")
```
...
```python
# %%
# Create a new column to identify Robinson helicopters
merged_list["robinson"] = merged_list["latimes_make"] == "ROBINSON"
print("Robinson column added")
```
...
```python
# %%
print("Merged list with Robinson column:")
...
```python
# %%
# Chart with Robinson highlighting
alt.Chart(merged_list).mark_bar().encode(
    x="per_100k_hours",
    y=alt.Y("latimes_make_and_model").sort("-x"),
    color="robinson"
...
```python
# %%
# Chart with Robinson highlighting, no legend
alt.Chart(merged_list).mark_bar().encode(
    x="per_100k_hours",
    y=alt.Y("latimes_make_and_model").sort("-x"),
